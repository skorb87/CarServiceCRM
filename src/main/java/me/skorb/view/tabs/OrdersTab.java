...
public class OrdersTab {
    ...
    private static final Logger logger = LogManager.getLogger(OrdersTab.class);
    ...
    public Tab createOrdersTab() {
        ordersTab = new Tab("Orders");

        initOrdersTabContentView();

        return ordersTab;
    }

    private void initOrdersTabContentView() {
        initOrdersTable();
        initLayout();
    }
...
//        });
    ...
    private void initLayout() {
        GridPane mainPane = new GridPane();
        mainPane.add(ordersTable, 0, 0);
        GridPane.setVgrow(ordersTable, Priority.ALWAYS);
        GridPane.setHgrow(ordersTable, Priority.ALWAYS);
        mainPane.setPrefWidth(Double.MAX_VALUE); // Ensure mainPane itself stretches across the full tab width

        ordersTab.setContent(mainPane);
    }
