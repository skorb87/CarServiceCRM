...
public class NewOrderView extends GridPane {

    private static final Logger logger = LogManager.getLogger(me.skorb.view.tabs.NewOrderTab.class);
...
//            selectedPhotos = new ArrayList<>(selectedFiles);
    ...
    private void initCreateOrderButton() {
        ...
        createOrderButton.setOnAction(event -> {
            ...
            if (selectedCustomer == null || selectedVehicle == null) {
                showToastPopup("Please, select Customer/Vehicle for a new order", ToastType.WARNING, createOrderButton);
                event.consume();
                return;
            }
            ...
            if (orderStatusBox.getValue() == null) {
            ...
            if (!errors.isEmpty()) {
                showToastPopup(errors, ToastType.ERROR, createOrderButton);
                event.consume();
                return;
            }
            ...
            // Assuming we are using modal window for order creation, so we are good to close it
            Platform.runLater(() -> {
                ((Stage) createOrderButton.getScene().getWindow()).close();
            });
    ...
    private void initLayout() {
        ...
        HBox.setHgrow(servicesPartsEmployeesLayout, Priority.ALWAYS);
        ...
        setVgap(10);
        add(topLayout, 0, 0);
        add(middleLayout, 0, 1);
        add(bottomLayout, 0, 2);
